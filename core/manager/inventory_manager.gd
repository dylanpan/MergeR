...
func load_from_data(items: Dictionary, currencies: Dictionary) -> void:
	_items = items.duplicate(true) if items else {}
	_currencies = currencies.duplicate(true) if currencies else {}

func save_to_data() -> Dictionary:
	return {
		"items": _items.duplicate(),
		"currencies": _currencies.duplicate()
	}
