...
func create_new_game_data(record_json = null, select_launchers: Array = [], select_order_self_id: int = 0, initial_cur_level: int = 0, initial_cur_round_idx: int = 0, runtime_map = null) -> Dictionary:
	...
	var init_data = {}
	init_data["launchers"] = {
		0: MetaConsts.launchers.get(select_launchers[0], {}) if select_launchers.size() > 0 else {},
		1: MetaConsts.launchers.get(select_launchers[1], {}) if select_launchers.size() > 1 else {},
	}
	init_data["elements"] = {}
	init_data["shots"] = {}
	...
	# 生成己方单位
	init_data["orderSelf"] = []
	var order_self_meta = MetaConsts.orderSelf.get(select_order_self_id, {})
